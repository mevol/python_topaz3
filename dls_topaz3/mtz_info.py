...
def mtz_get_xdata(mtz_filename):
    ...
    if (len(xdata) > 1):
        print(f"{len(xdata):d} lines of xdata found in {mtz_filename}, using first occurence")
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description='Get an MTZ file from the command line input.')

    parser.add_argument('--test_file', help='Use this option to use a test file.', action="store_true")

    parser.add_argument('mtz_filename', type=str, metavar='mtz_filename', help='MTZ file to get information from')
    ...
    if args.test_file:
        print("Testing")
        mtz_filename = '/dls/science/users/riw56156/topaz_test_data/AUTOMATIC_DEFAULT_free.mtz'
    else:
